...
def sign_up():
    if request.method=='POST':
        ...
        elif len (confirm_password)<7:
            flash('password must be greater than 7',category='error')
        elif new_password != confirm_password:
            flash('password don\'t match',category='error') 

        else:
            new_user=User(email=email,firstname=firstname,confirm_name=lastname,password=generate_password_hash(new_password,method='pbkdf2:sha256'))  
